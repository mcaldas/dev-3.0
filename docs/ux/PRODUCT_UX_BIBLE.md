...
- **Settings screen** (Global / Project): Global Settings uses a left-nav master-detail layout with nine Settings categories, localized entry search, and immediate RPC/local persistence; Project Settings keeps its existing tabs; destructive removal stays behind confirmation.
...
- **Settings category** — one of the nine Global Settings navigation items: Appearance, Tasks & Board, Keyboard, Terminal, Agents, Accounts, API models, Workspace, or System.
